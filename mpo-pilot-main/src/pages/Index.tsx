...
/** UHD showroom stills (4K-class WebP) — exec suite, auto gallery, museum, tech expo, etc. */
const BANNER_PHOTOS = SHOWROOM_HERO_UI;
...
// ── Project & compliance mode — PMBOK-oriented command surface ─────────────
function ProjectModeDashboard({
  firstName,
  kpis,
  nbaItems,
  criticalCount,
    ...
    <div className="flex flex-col min-h-screen bg-background">
      <div className="border-b px-6 py-4" style={{ borderColor: border, background: "hsl(var(--card))" }}>
        ...
        <h1 className="text-xl font-black text-foreground mt-1 tracking-tight">
          {firstName ? `${firstName}, delivery control center` : "Delivery control center"}
        </h1>
        <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
          Move fast like a startup while keeping PMBOK-style structure: boards, work packages, programs, compliance checkpoints, automations, and stakeholder CRM — audit-ready without losing task-level detail.
        </p>
        <div className="flex flex-wrap gap-2 mt-3">
          {[
            { label: "Board / WIP", to: "/agile", tone: "bg-primary/15 text-primary" },
            { label: "Compliance", to: "/compliance", tone: "bg-rose/10 text-rose" },
            { label: "PMBOK & playbooks", to: "/knowledge", tone: "bg-muted text-foreground" },
            { label: "Status reports", to: "/reports", tone: "bg-muted text-foreground" },
            { label: "Automations", to: "/workflows", tone: "bg-muted text-foreground" },
          ].map((x) => (
            <button
              key={x.to}
              type="button"
              onClick={() => navigate(x.to)}
              className={cn("text-xs font-bold px-3 py-1.5 rounded-full border transition-opacity hover:opacity-90", x.tone)}
              style={{ borderColor: "hsl(var(--border))" }}
            >
              {x.label}
            </button>
      ...
      <div className="flex-1 p-5 max-w-5xl mx-auto w-full space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: "On track", value: onTrackCount, signal: "green" as const, sub: "Healthy WIP" },
            { label: "Attention", value: atRiskCount + kpis.blocked, signal: "yellow" as const, sub: "Risk / blocked" },
            { label: "Critical", value: criticalCount, signal: "red" as const, sub: "Signals" },
            { label: "Open tasks", value: pendingActions, signal: "blue" as const, sub: "Action items" },
          ].map((k) => (
            <KpiTile key={k.label} label={k.label} value={k.value} sub={k.sub} signal={k.signal} />
          ))}
        </div>

        <div className="rounded-2xl border p-5" style={{ background: slate, borderColor: border }}>
          <p className="text-xs font-bold text-foreground mb-2">Compliance &amp; methodology checklist</p>
          <ul className="text-sm text-muted-foreground space-y-2 list-disc pl-5">
            <li>Log scope or requirements changes in <button type="button" className="font-semibold text-primary underline-offset-2 hover:underline" onClick={() => navigate("/decisions")}>Decisions</button>.</li>
            <li>Keep risk register and audits current under <button type="button" className="font-semibold text-primary underline-offset-2 hover:underline" onClick={() => navigate("/compliance")}>Compliance</button>.</li>
            <li>Align the team on milestones in <button type="button" className="font-semibold text-primary underline-offset-2 hover:underline" onClick={() => navigate("/initiatives")}>Programs</button> and <button type="button" className="font-semibold text-primary underline-offset-2 hover:underline" onClick={() => navigate("/projects")}>Work packages</button>.</li>
          </ul>
        </div>

        <div className="rounded-2xl border overflow-hidden" style={{ borderColor: border, background: "hsl(var(--card))" }}>
          <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: border }}>
            <span className="text-sm font-bold text-foreground">Next actions (personalized queue)</span>
            <button type="button" className="text-xs font-bold text-primary" onClick={() => navigate("/action-items")}>View all</button>
          </div>
          <div className="divide-y" style={{ borderColor: border }}>
            {nbaItems.length === 0 ? (
              <p className="p-6 text-sm text-muted-foreground">No queued actions — sync integrations or add tasks.</p>
            ) : (
              nbaItems.slice(0, 8).map((item, i) => (
                <div key={i} className="px-4 py-3 flex gap-3">
                  <span className="text-xs font-mono font-bold text-muted-foreground w-6">{i + 1}</span>
                  <div>
...
// ── Assisted mode dashboard (formerly Guided / Simple) ──────────────────────
    ...
    <div className="flex flex-col min-h-screen" style={{ background: "hsl(210 20% 98%)" }}>
      ...
      <div className="border-b px-8 py-6 flex items-center gap-5" style={{ background: "hsl(var(--card))", borderColor: "hsl(var(--border))" }}>
        ...
        <div className="flex-1">
          ...
          <p className="text-sm mt-0.5" style={{ color: "hsl(213 15% 52%)" }}>Assisted mode — hover sidebar items for short explanations</p>
        </div>
        <button onClick={() => setMode("executive")}
          ...
          <Rocket className="w-3.5 h-3.5" /> Executive layout
        </button>
      </div>
      ...
      <div className="flex-1 p-6 max-w-4xl mx-auto w-full space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="sm:col-span-2 grid grid-cols-2 gap-3">
            {[
              ...
              { label: "Open Actions", value: kpis.pendingActions, clr: "hsl(215 16% 42%)", bgClr: "hsl(215 20% 96%)", borderClr: "hsl(215 16% 86%)" },
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "My Work", icon: ListChecks, to: "/action-items", clr: "hsl(38 92% 44%)", bgClr: "hsl(38 92% 97%)", borderClr: "hsl(38 80% 84%)" },
            { label: "Programs", icon: Target, to: "/initiatives", clr: "hsl(152 60% 40%)", bgClr: "hsl(152 60% 96%)", borderClr: "hsl(152 55% 85%)" },
            { label: "Reports", icon: BarChart3, to: "/reports", clr: "hsl(215 16% 42%)", bgClr: "hsl(215 20% 96%)", borderClr: "hsl(215 16% 86%)" },
            { label: "Resources", icon: BookOpen, to: "/knowledge", clr: "hsl(215 16% 42%)", bgClr: "hsl(215 20% 96%)", borderClr: "hsl(215 16% 86%)" },
...
function CreativeDashboard({ firstName }: {
  ...
  return (
    <div className="flex flex-col" style={{ background: "#080808", minHeight: "100vh" }}>
            ...
            {C_PROJECTS.map((p, i) => {
              ...
              return (
                <button key={p.id}
                  ...
                  <img src={p.photoUrl} srcSet={p.photoSrcSet} sizes="200px" alt="" className="w-full h-full object-cover showroom-creative-img" />
                  {/* Overlay */}
                  <div className="absolute inset-0"
          ...
          {C_PROJECTS.map((p, i) => {
            ...
            return (
              <button key={p.id}
                ...
                <img src={p.photoUrl} srcSet={p.photoSrcSet} sizes="(max-width: 1024px) 50vw, 25vw" alt={p.title}
      ...
      {/* Social & marketing intelligence — wire integrations for live signals */}
      <div
        className="mx-4 sm:mx-8 mb-10 mt-2 rounded-2xl border px-5 sm:px-6 py-5"
        style={{
          borderColor: "rgba(255,255,255,0.08)",
          background: "linear-gradient(155deg, rgba(255,255,255,0.05) 0%, rgba(0,0,0,0.35) 100%)",
          boxShadow: "inset 0 1px 0 rgba(255,255,255,0.06)",
        }}>
        <p className="text-[9px] font-black uppercase tracking-[0.28em] mb-2" style={{ color: "rgba(255,255,255,0.4)" }}>
          Social &amp; algorithm pulse
        </p>
        <p className="text-sm leading-relaxed max-w-3xl" style={{ color: "rgba(255,255,255,0.72)" }}>
          Connect social accounts under{" "}
          <Link to="/integrations" className="font-semibold underline-offset-2 hover:underline" style={{ color: "#5eead4" }}>
            Integrations
          </Link>{" "}
          to ingest contacts, monitor reach, and spot what is working. Campaign projections and impact live in{" "}
          <Link to="/marketing" className="font-semibold underline-offset-2 hover:underline" style={{ color: "#c4b5fd" }}>
            Outreach
          </Link>
          . Feed carousels can surface inspiration from connected networks once OAuth is enabled.
        </p>
        <div className="flex flex-wrap gap-2 mt-4">
          <Link
            to="/crm"
            className="text-[11px] font-bold px-3 py-1.5 rounded-full border transition-colors hover:bg-white/5"
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ background: dark }}>
      ...
      <div className="border-b px-7 py-2.5 flex flex-wrap items-center gap-x-5 gap-y-1 text-[10px] font-semibold uppercase tracking-widest"
        ...
        <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full" style={{ background: "hsl(0 72% 58%)" }} /> Act now</span>
      </div>
...
export default function Dashboard() {
  ...
  const { user } = useAuth();
  const { mode, isSimpleMode, isStartupProjectMode } = useUserMode();
  ...
  const iniTotal = Math.max(1, data.initiatives.length);
  const onTrackBarPct = Math.min(100, Math.round((onTrackCount / iniTotal) * 100));
  const actionLoadPct = Math.min(100, Math.round((pendingActions / Math.max(8, pendingActions + completedCount)) * 100));
  const riskBarPct = Math.min(100, Math.round(((atRiskCount + criticalCount) / iniTotal) * 100));
  const orgHealthPct = Math.round(liveOverallHealth ?? 0);
          ...
          metrics={[
            {
              ...
              label: "Org health",
              value: `${orgHealthPct}%`,
              sub: "Composite from diagnostics & execution",
              barPct: orgHealthPct,
              accent: orgHealthPct >= 60 ? "blue" : "red",
              href: "/diagnostics",
            },
            {
              id: "ontrack",
              ...
              sub: `${iniTotal} initiatives in portfolio`,
              barPct: onTrackBarPct,
              accent: "black",
              href: "/initiatives",
            },
            {
              ...
              sub: "Tasks in flight across teams",
              barPct: actionLoadPct,
              accent: "red",
              href: "/action-items",
            },
            {
              ...
              value: String(atRiskCount + criticalCount),
              sub: "At risk + critical signals",
              barPct: riskBarPct,
              accent: "red",
              href: "/diagnostics",
            },
