...
import {
  ...
  Cpu, ClipboardCheck, AppWindow,
} from "lucide-react";
import pmoLogoNew from "@/assets/pmo-logo-new.png";
import { useUserMode } from "@/hooks/useUserMode";
import { MODE_NAV_HINTS } from "@/lib/modeNavHints";
...
const MODE_NAV_CONFIGS: Record<string, ModeNavConfig> = {
  ...
  founder: {
    commandLabel: "Oversight",
    workLabel: "Operations",
    growthLabel: "Pipeline",
    toolsLabel: "Finance & admin",
    workAlwaysOpen: false,
    command: [
      { to: "/",            label: "Dashboard", icon: LayoutDashboard },
      { to: "/decisions",   label: "Decisions", icon: Scale           },
  ...
  startup: {
    ...
    workLabel: "Delivery",
    growthLabel: "Revenue & stakeholders",
    toolsLabel: "Platform & governance",
    workAlwaysOpen: true,
    command: [
      { to: "/",            label: "Dashboard", icon: LayoutDashboard },
      { to: "/reports",     label: "Reports",   icon: FileText        },
    ...
    work: [
      { to: "/agile",        label: "Board / WIP", icon: Layers      },
      { to: "/projects",     label: "Projects",    icon: FolderOpen  },
      ...
      { to: "/workflows",    label: "Automations", icon: GitBranch   },
    ],
    growth: [
    ...
    tools: [
      ...
      { to: "/diagnostics",  label: "Diagnostics",       icon: Activity   },
      { to: "/advisory",     label: "PMO advisory",      icon: Headphones },
  ...
  healthcare: {
    commandLabel: "Practice",
    workLabel: "Operations",
    growthLabel: "Outreach",
    toolsLabel: "Clinical & finance",
    workAlwaysOpen: false,
    command: [
      { to: "/",            label: "Dashboard", icon: LayoutDashboard },
      { to: "/decisions",   label: "Decisions", icon: Scale           },
...
export default function AppLayout({ children, profile, onProfileUpdate }: Props) {
  ...
  useEffect(() => {
    const label =
      ...
      (location.pathname.startsWith("/miiddle") ? "Miiddle" :
        location.pathname.startsWith("/tech-ops") ? "Tech-Ops" :
        "Dashboard");
    try {
  ...
  const navHint = (path: string, label: string) =>
    mode === "simple" ? (MODE_NAV_HINTS[path] ?? `Open ${label}.`) : undefined;
  ...
  return (
    <div className="pmo-grid" style={{
      ...
      <main
        ...
        {mode === "healthcare" && (
          <div
            className="flex flex-col gap-1 px-4 py-3 text-sm border-b"
            style={{
              background: "hsl(186 45% 94%)",
              borderColor: "hsl(var(--border))",
              color: "hsl(200 25% 22%)",
            }}
            role="status"
          >
            <span className="font-bold">Healthcare mode</span>
            <span className="text-xs leading-relaxed opacity-90">
              Built for HIPAA-aware operations. Use Compliance for policies, Integrations for practice management and accounting connections,
              and avoid putting PHI in unsecured notes. This product does not replace your BAA-covered vendors — configure integrations accordingly.
            </span>
          </div>
        )}
        {children}
