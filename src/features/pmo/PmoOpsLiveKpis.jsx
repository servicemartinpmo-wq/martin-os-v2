...
export default function PmoOpsLiveKpis() {
  ...
  return (
    <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {kpis.map((kpi) => (
        <article key={kpi.label} className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
          <p className="text-xs uppercase tracking-wide text-zinc-400">{kpi.label}</p>
          <p className="mt-2 text-2xl font-semibold text-zinc-100">{busy ? '…' : kpi.value}</p>
