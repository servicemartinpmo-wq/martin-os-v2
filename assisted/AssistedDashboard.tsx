...
function AssistedCard({ title, description, icon: Icon, status, actionId, onAction, disabled }: AssistedCardProps) {
  const statusColors = {
  ...
  return (
    <button 
      ...
      disabled={disabled}
      className="w-full bg-white p-8 rounded-[2rem] border-4 border-slate-100 hover:border-blue-400 transition-all text-left group relative disabled:opacity-50"
    >
    ...
    <div className="p-12 space-y-12 max-w-6xl mx-auto">
      <header className="flex justify-between items-center bg-blue-600 p-12 rounded-[3rem] text-white shadow-2xl shadow-blue-200">
        ...
        <div className="flex gap-4">
          <button 
            onClick={() => handleAction('get_help')}
            disabled={actionLoading}
            className="p-6 bg-white/20 rounded-full hover:bg-white/30 transition-colors disabled:opacity-50"
          >
      ...
      <div className="bg-slate-50 p-12 rounded-[3rem] border-4 border-dashed border-slate-200">
        ...
        <button 
          onClick={() => handleAction('call_support')}
          disabled={actionLoading}
          className="px-12 py-6 bg-slate-900 text-white rounded-full text-2xl font-bold hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 disabled:opacity-50"
        >
          Call Support Now
        </button>
      </div>
