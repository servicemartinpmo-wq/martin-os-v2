...
export default function LockscreenBanner({ mode, className }: LockscreenBannerProps) {
  ...
  // Pool of high-quality Pexels 4K assets
  const allAssets = [
    `https://images.pexels.com/photos/355508/pexels-photo-355508.jpeg?auto=compress&cs=tinysrgb&w=3840&h=2160&dpr=1`,
    `https://images.pexels.com/photos/1181244/pexels-photo-1181244.jpeg?auto=compress&cs=tinysrgb&w=3840&h=2160&dpr=1`,
    `https://images.pexels.com/photos/204375/pexels-photo-204375.jpeg?auto=compress&cs=tinysrgb&w=3840&h=2160&dpr=1`,
    `https://videos.pexels.com/video-files/3196396/3196396-uhd_3840_2160_25fps.mp4`,
    `https://videos.pexels.com/video-files/4661877/4661877-uhd_3840_2160_25fps.mp4`,
    `https://images.pexels.com/photos/3244513/pexels-photo-3244513.jpeg?auto=compress&cs=tinysrgb&w=3840&h=2160&dpr=1`,
  ];

  // Daily rotation logic
  const today = new Date().toISOString().split('T')[0];
  const startIndex = parseInt(today.replace(/-/g, '')) % allAssets.length;
  
  // Select 3 assets starting from startIndex
  const mediaAssets = [
    allAssets[startIndex % allAssets.length],
    allAssets[(startIndex + 1) % allAssets.length],
    allAssets[(startIndex + 2) % allAssets.length],
  ];

  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentImageIndex((prev) => (prev + 1) % mediaAssets.length);
    }, 10000); // Change asset every 10 seconds
    ...
    <div className={cn("relative w-full rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/10 h-[500px] md:h-auto", className)}>
      ...
      <div className="absolute inset-0 z-0">
        <AnimatePresence mode="wait">
          ...
          >
            {mediaAssets[currentImageIndex].endsWith('.mp4') ? (
              <video
                src={mediaAssets[currentImageIndex]}
                autoPlay
                muted
                loop
                playsInline
