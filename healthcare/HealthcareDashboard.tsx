    ...
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      <header className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
          <button 
            onClick={() => handleAction('find_patient')}
          ...
          <button 
            onClick={() => handleAction('new_encounter')}
            disabled={actionLoading}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { id: 'daily_revenue', label: 'Daily Revenue', value: '$12,450', trend: '+8%', icon: DollarSign },
          { id: 'patient_volume', label: 'Patient Volume', value: '42', trend: '+12', icon: Users },
          { id: 'billing_health', label: 'Billing Health', value: '96.2%', trend: '+0.5%', icon: Activity },
          { id: 'open_claims', label: 'Open Claims', value: '18', trend: '$4.2k', icon: FileText },
        ].map((stat, i) => (
          <div 
            key={i} 
            className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm cursor-pointer hover:border-green-200 transition-all"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            {[
              { id: 'p1', name: 'Sarah Miller', time: '09:00 AM', type: 'Follow-up', status: 'Checked In' },
              { id: 'p2', name: 'John Smith', time: '10:30 AM', type: 'Initial Consultation', status: 'Waiting' },
              { id: 'p3', name: 'Emily Davis', time: '11:15 AM', type: 'Lab Review', status: 'In Progress' },
              { id: 'p4', name: 'Michael Brown', time: '01:00 PM', type: 'Annual Physical', status: 'Scheduled' },
            ].map((patient, i) => (
              <div 
                key={i} 
                className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all group flex items-center justify-between cursor-pointer"
                onClick={() => handleAction('view_patient_detail', { id: patient.id })}
              >
                <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <button 
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      handleAction('patient_options', { id: patient.id });
                    }}
                  >
        ...
        <div className="space-y-8">
          <div className="bg-slate-900 rounded-[2rem] p-8 text-white">
            ...
            <button 
              ...
              disabled={actionLoading}
              className="w-full mt-8 py-3 bg-white/10 text-white rounded-xl text-xs font-bold hover:bg-white/20 transition-colors border border-white/10 disabled:opacity-50"
            >
              Run Full Audit
            </button>
          </div>
          ...
          <div 
            className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm cursor-pointer hover:bg-slate-50 transition-all"
            onClick={() => handleAction('view_clinical_insights')}
          >
