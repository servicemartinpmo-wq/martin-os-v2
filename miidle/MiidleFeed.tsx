...
export default function MiidleFeed({ mode }: MiidleFeedProps) {
  ...
  // Fallback to mock data if no stories in DB for demonstration
  const displayStories = stories.length > 0 ? stories : [
    {
      ...
      outcome_metrics: { 'Time Saved': '12h/week', 'Accuracy': '99.9%' },
      status: 'Published',
      phases: [
    ...
    },
    {
      ...
      summary: 'Re-architecting the frontend to improve load times by 40%.',
      tags: ['Frontend', 'Performance', 'UX'],
      outcome_metrics: { 'Load Time': '-40%', 'Conversion': '+15%' },
      status: 'Published',
      phases: [
        { id: 'p1', title: 'Idea', description: 'Users were dropping off due to slow initial load times.', status: 'Completed' },
        { id: 'p2', title: 'Middle', description: 'Migrating to a new framework and optimizing asset delivery.', status: 'In Progress' },
        { id: 'p3', title: 'Execution', description: 'A/B testing the new portal with a subset of users.', status: 'Not Started' },
      ]
    }
  ];
          ...
          {displayStories.map((story) => (
            <div key={story.id} className="relative pl-8 border-l-2 border-slate-100 pb-6 last:pb-0">
              ...
              <div className="flex justify-between items-start mb-1">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">#{String(story.id).slice(0, 8)}</span>
              </div>
            ...
            <div className="p-8 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center font-bold text-slate-400 border border-slate-200">
                  {String(story.id).slice(-2)}
                </div>
                <div>
                  <h4 className="font-bold text-slate-900 text-lg">Build #{String(story.id).slice(0, 8)}</h4>
